  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[600px] max-h-[90vh] p-0 gap-0 bg-white">
        ...
        {/* 헤더 - 닫기 버튼만 */}
        <div className="flex items-center justify-end px-6 py-4 bg-white rounded-t-2xl">
          <button
            onClick={() => onOpenChange(false)}
            className="w-9 h-9 flex items-center justify-center bg-[#F6F6EA] rounded-lg hover:bg-[#EEEEDE] transition-colors"
          >
            <X className="w-7 h-7 text-grayscale-gray5" />
          </button>
        </div>
                ...
                {application.status === 'consultation_completed' ? (
                  ...
                  <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#A0C8F4] rounded-full">
                    <div className="w-4 h-4 flex items-center justify-center">
                      <div className="w-2 h-0.5 bg-[#4F3B2E]" />
                    </div>
                  ...
                  answer={
                    <div className="flex flex-col gap-2.5">
                      <TextAreaField
                        ...
                        maxCount={800}
                      />
                      <p className="text-xs text-[#888]">아이들은 철장, 베란다, 야외 등 열악한 공간에서는 지낼 수 없어요</p>
                    </div>
                  }
                />
          ...
          <div className="bg-white px-6 py-4 rounded-b-2xl flex items-center justify-between">
            {application.status !== 'consultation_completed' && (
              <div className="flex items-center gap-1">
                <Info className="w-3 h-3 text-[#A0A0A0]" />
                <p className="text-xs text-[#A0A0A0]">상담이 끝나면 [상담 완료]를 눌러 상대방이 후기를 남길 수 있게 해주세요.</p>
              </div>
            )}
            {application.status === 'consultation_completed' ? (
              <Button
...
const InfoText = ({ text }: { text: string }) => (
  <div className="flex items-start gap-1">
    <div className="w-0.5 h-3 bg-[#888] shrink-0" />

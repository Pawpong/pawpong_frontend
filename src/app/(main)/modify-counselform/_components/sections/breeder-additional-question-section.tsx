          ...
          <div className="flex flex-col w-full gap-8">
            {fields.map((field, index) => (
              <div key={field.id} className="flex flex-col w-full gap-3">
                ...
                <Controller
                  name={`breederQuestions.${index}.question`}
                  control={control}
                  render={({ field }) => (
                    <Input
                      {...field}
                      ...
                      data-breeder-question-input="true"
                      className="h-12 w-full rounded-[--spacing(2)] bg-white px-4 py-3 text-body-s font-medium text-color-primary-500-basic leading-body-s placeholder:text-grayscale-gray4 focus:outline-none pointer-events-auto cursor-text"
                    />
                  )}
                />
                <textarea

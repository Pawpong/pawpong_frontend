...
export const getVideoComments = async (
  ...
  success: boolean;
  code: number;
  data: { items: FeedComment[]; totalCount: number; hasNextPage: boolean };
}> => {
  const response = await api.get<FeedCommentsResponse>(`/api/feed/comment/${videoId}?page=${page}&limit=${limit}`);

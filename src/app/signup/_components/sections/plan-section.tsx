...
export default function PlanSection() {
  ...
  return (
    <SignupFormSection>
      ...
      <SignupFormItems>
        {planInfo.map(({ name, label, description, src, className, disabled, isFree }) => (
          <Button
            disabled={disabled}
            key={name}
            className={cn('w-full justify-between py-7 px-8  rounded-2xl gap-5 disabled:opacity-100', className)}
            onClick={() => {
              setPlan(name);
              nextFlowIndex();
            }}

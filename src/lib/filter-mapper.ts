...
export function mapFiltersToParams(
  ...
  if (petType === "dog") {
    const dogSizes = activeFilters
      .filter((filter) => filter in DOG_SIZE_MAP)
      .map((filter) => DOG_SIZE_MAP[filter]);
  ...
  activeFilters.forEach((filter) => {
    ...
    // 반드시 KOREA_PROVINCES로 시작해야 지역으로 인식
      ...
      const hasProvincePrefix = KOREA_PROVINCES.some((province) =>
        filter.startsWith(province + " ")
      );
  ...
  const breeds = activeFilters.filter((filter) => {
    ...
    const isProvinceCity = KOREA_PROVINCES.some((province) =>
      filter.startsWith(province + " ")
    );
